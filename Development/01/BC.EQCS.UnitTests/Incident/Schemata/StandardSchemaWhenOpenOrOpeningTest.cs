...
namespace BC.EQCS.UnitTests.Incident.Schemata
{
    ...
    // And incident is STANDARD form
    // And incident is being raised
    [TestFixture(null, IncidentCommand.Raise)]
    // Given incident is new
    // And incident is STANDARD form
    // And incident is being raised
    [TestFixture(IncidentStatus.Submitted, null)]
    // Given incident is Raised
    // And incident is STANDARD form
    [TestFixture(IncidentStatus.Submitted, IncidentCommand.Save)]
    // Given incident is Raised
    // And incident is STANDARD form
    // And incident is being updated
    [TestFixture(IncidentStatus.Submitted, IncidentCommand.Accept)]
    // Given incident is Raised
    // And incident is STANDARD form
    // And incident is being Accepted
    [TestFixture(IncidentStatus.InProgress, null)]
    // Given incident is InProgress
    // And incident is STANDARD form
    [TestFixture(IncidentStatus.InProgress, IncidentCommand.Save)]
    // Given incident is InProgress
    // And incident is STANDARD form
    // And incident is being updated
    public class StandardSchemaWhenOpenOrOpeningTest : IncidentSchemaBuildTest
    {
        private readonly IncidentCommand? _command;
        private readonly IncidentStatus? _currentStatus;

        public StandardSchemaWhenOpenOrOpeningTest(IncidentStatus? currentStatus, IncidentCommand? command)
        {
