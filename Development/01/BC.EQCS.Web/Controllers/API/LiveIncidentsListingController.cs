...
{
    public class LiveIncidentsListingController : ODataController
    {
        ...
        public LiveIncidentsListingController(IOdataRepository<IncidentsListingModel> incidentOdataRepository,
        ...
        [Authorize]
        public async Task<IHttpActionResult> GetLiveIncidentsListing(
            ODataQueryOptions<IncidentsListingModel> queryOptions)
        {
            if (_authoriser.IsAuthorised(AssetType.IncidentViewListIncidents))
            {
                ...
                var itemsToreturn = ((IncidentListingODataRepository) _incidentOdataRepository)
                    .GetLiveIncidentsList(queryOptions, areas, categoriesThisUserCanView, _context.CurrentUser);
